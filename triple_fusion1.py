...
def get_stable_temp(is_calib=False):
    ...
    temp_diff = -math.log(ratio) * SENSITIVITY
...
def audio_loop():
    ...
    for i in range(p.get_device_count()):
        if any(k in p.get_device_info_by_index(i)['name'] for k in ["OnePlus", "bluez", "Buds"]):
            target_id = i; break
...
def run_diagnosis(animal, temp, cough, lethargy):
    try:
        ...
        for disease, count in results.items():
            ...
            if prob < 5: continue # Only show relevant matches
            report += f"  → {disease}: {prob:.1f}% Match\n"
            
            # Extract secondary symptoms for insight
...
print("\n🚀 AUTONOMOUS AI ACTIVE: Calculating results every 10 seconds.")
print("Monitoring: Temperature, Motion, and Audio Patterns.\n")
...
try:
    while True:
        ...
        print("-" * 50)
        print(f"⏰ TIME: {time.strftime('%H:%M:%S')} | SENSORS: Temp={t}°C, Cough={c}, Lethargy={l}")
        print("-" * 50)
        
        # Run for Cow
        print(f"🐄 COW ANALYSIS:")
        print(run_diagnosis("cow", t, c, l))
        
        # Run for Goat
        print(f"🐐 GOAT ANALYSIS:")
        print(run_diagnosis("goat", t, c, l))
        
        # Reset latch and wait 10 seconds
